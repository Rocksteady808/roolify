...
export async function POST(req: Request) {
  try {
    console.log('[Fix with Stable IDs] Creating notification settings with stable identifiers...');
    
    // Delete the current settings (ID 54)
    await xanoNotifications.delete(54);
    console.log('[Fix with Stable IDs] ✅ Deleted old settings');
    
    // Create new settings using STABLE identifiers that never change
    const newSettings = await xanoNotifications.create({
      // Use stable identifiers instead of numeric IDs
      site_id: '652b10ed79cbf4ed07a349ed',        // This never changes
      html_form_id: '68f648d669c38f6206830743',   // This never changes
      user_id: 1,                                  // This is stable
      admin_routes: [
        {
    ...
    console.log('[Fix with Stable IDs] ✅ Created settings with stable identifiers');
    
    return NextResponse.json({
      success: true,
      message: 'Notification settings created with stable identifiers',
      settings: newSettings
    });
    
  } catch (error: any) {
    console.error('[Fix with Stable IDs] ❌ Error:', error);
    return NextResponse.json({
      error: error.message || 'Failed to create settings with stable IDs'
    }, { status: 500 });
  }
}
