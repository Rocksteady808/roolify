...
export async function POST(req: Request) {
  try {
    console.log('[Fix Form 57 Final] Deleting old settings and creating new ones for form 105...');
    
    // Delete the current settings (ID 57)
    await xanoNotifications.delete(57);
    console.log('[Fix Form 57 Final] ✅ Deleted old settings');
    
    // Create new settings for form 105 (the one actually receiving submissions) with your custom values
    const newSettings = await xanoNotifications.create({
      ...
      user_routes: [],
      admin_fallback_email: 'aarontownsend6@gmail.com', // Your fallback email
      user_fallback_email: '',
      custom_value: '', // No global custom value
      field_custom_values: {
        'Privacy Policy': '<a href="https://www.hbiinnovations.com/privacy-policy">Privacy Policy</a>',
        'Terms Of Service': '<a href="https://www.hbiinnovations.com/terms-of-services">Terms Of Service</a>'
      },
    ...
    console.log('[Fix Form 57 Final] ✅ Created new settings for form 105 with your custom values');
    
    return NextResponse.json({
      success: true,
      message: 'Notification settings created for form 105 with your custom values',
      settings: newSettings
    });
    
  } catch (error: any) {
    console.error('[Fix Form 57 Final] ❌ Error:', error);
    return NextResponse.json({
