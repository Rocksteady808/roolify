...
function initializeIframe() {
  // Get the iframe
  state.iframe = document.getElementById('mainIframe') as HTMLIFrameElement;
...
function updateUserInfo(userData: { name?: string; email?: string }) {
...
function setupUserDropdown() {
  const dropdownBtn = document.querySelector('.user-dropdown-btn') as HTMLButtonElement;
  const dropdownMenu = document.querySelector('.user-dropdown-menu') as HTMLDivElement;
  ...
  document.addEventListener('click', (e) => {
    if (isOpen && !dropdownMenu.contains(e.target as Node) && !dropdownBtn.contains(e.target as Node)) {
      isOpen = false;
      dropdownMenu.style.display = 'none';
      dropdownBtn.style.backgroundColor = 'transparent';
    }
...
async function fetchAndSendCurrentSiteInfo() {
  try {
...
(window as any).RoolifyExtension = {
  state
};